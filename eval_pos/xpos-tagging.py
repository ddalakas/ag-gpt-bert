...
XPoS tagging script reading conllu data and a config.py file to perform a wandb run.
"""
...
def pos_processor(sentence):
    ...
    sent_labels = [token["xpos"] for token in sentence]
...
def train(config, hybrid_name, seed=42, task_name="xpos"):
    """
...
if len(sys.argv) < 2:
    print(f"Usage: python xpos-tagging.py xpos-config.py [seed]")
    sys.exit(1)
