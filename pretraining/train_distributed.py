...
from dataset import MaskedDataset, CausalDataset, MaskedValidationDataset, CausalValidationDataset
...
def safe_print(*args, rank=None, **kwargs):
    prefix = f"[RANK {rank}] " if rank is not None else ""
    with print_lock:
        print(prefix, *args, **kwargs, flush=True, file=sys.stdout)

os.environ["WANDB_MODE"] = "offline"

def cleanup_and_exit(signum, frame):
    safe_print("Received termination signal. Finishing wandb...")
    wandb.finish()
    sys.exit(0)

# Catch SIGTERM (job time limit) and SIGINT
...
def parse_arguments():
    ...
    parser.add_argument("--train_path", default="../bin/train_tokenized.bin", type=Path, help="Path to the tokenized training data.")
    parser.add_argument("--valid_path", default="../bin/val_tokenized.bin", type=Path, help="Path to the tokenized validation data.")
    parser.add_argument("--name", default="Test Run", type=str, help="Name of the run.")
    parser.add_argument("--wandb_project", type=str, help="Name of the WandB project to log into.")
    parser.add_argument("--wandb_entity", type=str, help="The entity to log to on WandB.")
    parser.add_argument("--config_file", default="../configs/config.json", type=Path, help="The BERT model config")
    parser.add_argument("--tokenizer_path", default="../tokenizer/tokenizer.json", type=Path, help="Path to the tokenizer.")
    ...
    parser.add_argument("--hybrid_numerator", default=2, type=int, help="The numerator of the hybrid ratio.")
    ...
    parser.add_argument("--local_batch_size", default=32, type=int, help="Batch size for training per GPU.")
    ...
    parser.add_argument("--max_steps", default=40000, type=int, help="Total number of training steps to perform.")
    parser.add_argument("--ema_decay", default=0.999, type=float, help="Exponential moving average decay.")
    parser.add_argument("--validate_every", default=1000, type=int, help="Run validation after every X training shards.")
    ...
    parser.add_argument('--save_every', type=int, default=1000, help="save every X steps")
    ...
    args.output_path = args.output_dir / f"{args.name}.bin"
...
def setup_training(args, tokenizer):
    ...
    safe_print(f"Hello from rank {args.rank} of {args.world_size} on {gethostname()}. Dataset type: {args.dataset_type}")
    ...
    if args.rank == 0:
        safe_print(f"Group initialized? {torch.distributed.is_initialized()}")

    args.local_rank = args.rank - args.gpus_per_node * (args.rank // args.gpus_per_node)
    torch.cuda.set_device(args.local_rank)
    args.device = torch.device("cuda", args.local_rank)
    safe_print(f"RCCL started on device {args.device}")
    safe_print(f"host: {gethostname()}, rank: {args.rank}, local_rank: {args.local_rank}")
...
def training_epoch(model, ema_model, train_dataloader, masked_valid_dataloader, causal_valid_dataloader, optimizer, scheduler, global_step, epoch, args):
    model = model.train()
    ...
    for local_step in tqdm(range(num_steps), desc="Train iteration", initial=global_step, total=args.max_steps, disable=not is_main_process()):
        ...
        with torch.amp.autocast(device_type='cuda', dtype=torch.bfloat16, enabled=args.mixed_precision):
            ...
            wandb.log(
                {
                    ...
                    "stats/seq_length": train_dataloader._dataset.seq_length,
        ...
        if (global_step + 1) % args.validate_every == 0:
            validation_epoch(model, masked_valid_dataloader, causal_valid_dataloader, epoch, args)
            model.train()
...
@torch.no_grad()
def validation_epoch(model, masked_valid_dataloader, causal_valid_dataloader, epoch, args, commit=False):
    model = model.eval()

    # MLM Validation
    mlm_losses, mlm_accuracies = [], []
    masked_valid_dataloader_iter = iter(masked_valid_dataloader)
    input_ids, attention_mask, target_ids, _ = get_batch(masked_valid_dataloader_iter, args.device, 0)

    for local_step in tqdm(range(args.validation_steps), desc="MLM Valid iteration", disable=not is_main_process()):
        with torch.amp.autocast(device_type='cuda', dtype=torch.bfloat16, enabled=args.mixed_precision):
        ...
        mlm_accuracies.append(accuracy)

    # Causal Validation
    causal_losses, causal_accuracies = [], []
    causal_valid_dataloader_iter = iter(causal_valid_dataloader)
    input_ids, attention_mask, target_ids, _ = get_batch(causal_valid_dataloader_iter, args.device, 0)

    for local_step in tqdm(range(args.validation_steps), desc="Causal Valid iteration", disable=not is_main_process()):
        with torch.amp.autocast(device_type='cuda', dtype=torch.bfloat16, enabled=args.mixed_precision):
            loss, accuracy, _, num_tokens = model(input_ids, attention_mask, target_ids)
        ...
        causal_losses.append(loss)
        causal_accuracies.append(accuracy)

    if is_main_process():
        wandb.log(
            {
                "validation/mlm_loss": mean(mlm_losses),
                "validation/mlm_accuracy": mean(mlm_accuracies) * 100.0,
                "validation/mlm_perplexity": math.exp(mean(mlm_losses)),
                "validation/causal_loss": mean(causal_losses),
                "validation/causal_accuracy": mean(causal_accuracies) * 100.0,
                "validation/causal_perplexity": math.exp(mean(causal_losses))
            },
            commit=commit
        )
...
def save(model, ema_model, optimizer, scheduler, global_step, epoch, args):
    if is_main_process():
        ...
        ema_path = args.output_dir / f"{args.name}_ema.bin"
        torch.save(ema_model.state_dict(), ema_path)

        state_path = args.output_dir / f"{args.name}_state_dict.bin"
        torch.save(
            {
...
def load_datasets(args, tokenizer, epoch, global_step, train_dataloader, masked_valid_dataloader, causal_valid_dataloader):
    train_seed = args.seed + get_rank() + epoch * get_world_size()

    # Use static sequence length
    ...
    # Create both validation dataloaders
    if masked_valid_dataloader is None:
        masked_valid_data = MaskedValidationDataset(args.valid_path, tokenizer, args)
        masked_valid_dataloader = DataLoader(
            masked_valid_data,
            shuffle=False,
    ...
    if causal_valid_dataloader is None:
        causal_valid_data = CausalValidationDataset(args.valid_path, tokenizer, args)
        causal_valid_dataloader = DataLoader(
            causal_valid_data,
            shuffle=False,
            batch_size=args.local_batch_size,
            num_workers=0,
    ...
    return train_dataloader, masked_valid_dataloader, causal_valid_dataloader
...
if __name__ == "__main__":
    ...
    for epoch in count(start=start_epoch):
        ...
        global_step = training_epoch(
            model, ema_model, train_dataloader, masked_valid_dataloader, causal_valid_dataloader,
            optimizer, scheduler, global_step, epoch, args
        )

        safe_print(
            f"Dataset type: {args.dataset_type}, Epoch count: {epoch}, "
    ...
    save(model, ema_model, optimizer, scheduler, global_step, epoch, args)
    validation_epoch(model, masked_valid_dataloader, causal_valid_dataloader, epoch, args, commit=True)